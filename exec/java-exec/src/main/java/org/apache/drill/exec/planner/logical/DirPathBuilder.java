...
public class DirPathBuilder extends RexVisitorImpl <SchemaPath> {
  ...
  private void initPathComponents() {
    int maxHierarychy = partitionDescriptor.getMaxHierarychyLevel();
    dirNameList = Lists.newArrayListWithExpectedSize(maxHierarychy);
    conjunctList = Lists.newArrayListWithExpectedSize(maxHierarychy);
    for (int i=0; i < maxHierarychy; i++) {
    ...
    case BINARY:
      if (call.getKind() == SqlKind.EQUALS) {
        ...
        if (dirMatch && e1 != null) {
          ...
          if (hierarychyIndex >= partitionDescriptor.getMaxHierarychyLevel()) {
            return null;
          }
  ...
  private SchemaPath getInputFromCast(RexCall call){
    ...
    if (arg != null && partitionDescriptor.isPartitionName(arg.getRootSegment().getPath())) {
      dirMatch = true;
      return arg;
    }
    return null;
