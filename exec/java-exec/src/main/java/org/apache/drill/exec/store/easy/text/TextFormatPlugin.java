...
public class TextFormatPlugin extends EasyFormatPlugin<TextFormatPlugin.TextFormatConfig> {
  ...
  public RecordReader getRecordReader(FragmentContext context, FileWork fileWork,
    ...
    char delim = ((TextFormatConfig) formatConfig).getDelimiter().charAt(0);

    if (context.getOptions().getOption(ExecConstants.ENABLE_NEW_TEXT_READER_KEY).bool_val == true) {
      TextParsingSettings s = new TextParsingSettings();
      s.setDelimiter((byte) delim);
      return new CompliantTextRecordReader(split, context, s, columns);
    } else {
      return new DrillTextRecordReader(split, context, delim, columns);
    }
  }
