...
public class StringFunctions{
  ...
  public static class Like implements DrillSimpleFunc{
    ...
    public void setup(RecordBatch incoming){
      matcher = java.util.regex.Pattern.compile(org.apache.drill.exec.expr.fn.impl.RegexpUtil.sqlToRegexLike(VarCharHolder.toString(pattern.start,  pattern.end,  pattern.buffer))).matcher("");
    }

    public void eval(){
      String i = VarCharHolder.toString(input.start, input.end, input.buffer);
      matcher.reset(i);
  ...
  public static class Similar implements DrillSimpleFunc{
    ...
    public void setup(RecordBatch incoming){
      ...
      matcher = java.util.regex.Pattern.compile(org.apache.drill.exec.expr.fn.impl.RegexpUtil.sqlToRegexSimilar(VarCharHolder.toString(pattern.start,  pattern.end,  pattern.buffer))).matcher("");
    }
  ...
  public static class RegexpReplace implements DrillSimpleFunc{
    ...
    public void setup(RecordBatch incoming){
      matcher = java.util.regex.Pattern.compile(VarCharHolder.toString(pattern.start,  pattern.end,  pattern.buffer)).matcher("");
    }
    ...
    public void eval(){
      ...
      out.start = 0;
      String i = VarCharHolder.toString(input.start, input.end, input.buffer);
      String r = VarCharHolder.toString(replacement.start, replacement.end, replacement.buffer);
  ...
  public static class InitCap implements DrillSimpleFunc{
    ...
    public void eval() {
      ...
      out.start = 0;
      out.end = input.end - input.start;
      StringFunctionHelpers.initCap(input.start, input.end, input.buffer, out.buffer);
    }
  ...
  */
  ...
  public static class RepeatString implements DrillSimpleFunc {
    ...
    public void eval() {
      ...
      out.start = 0;
      out.buffer = buffer = buffer.reallocIfNeeded( len * nTimes.value );
      int num = nTimes.value;
      for(int i =0; i < num; i++){
        in.buffer.getBytes(in.start, out.buffer, num * len, len);
      }
      out.end = len*nTimes.value;
    }
  }
  ...
  */
  ...
  public static class AsciiEndode implements DrillSimpleFunc {
    ...
    public void setup(RecordBatch incoming) {
      inCharset = java.nio.charset.Charset.forName(VarCharHolder.toString(enc.start, enc.end, enc.buffer));
    }
