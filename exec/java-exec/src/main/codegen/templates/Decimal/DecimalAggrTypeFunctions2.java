...
public static class ${type.inputType}${aggrtype.className} implements DrillAggFunc{
  ...
  @Workspace ${type.sumRunningType}Holder value;
  @Workspace ${type.countRunningType}Holder count;
  @Workspace ByteBuf buffer;
  @Output ${type.outputType}Holder out;

  public void setup(RecordBatch b) {
    value = new ${type.sumRunningType}Holder();
  ...
  public void output() {
    java.math.BigDecimal runningSum = org.apache.drill.common.util.DecimalUtility.getBigDecimalFromSparse(value.buffer, value.start, value.nDecimalDigits, value.scale);
    java.math.BigDecimal average = runningSum.divide(java.math.BigDecimal.valueOf(count.value, 0), value.scale, java.math.BigDecimal.ROUND_HALF_UP);
    for (int i = 0; i < value.nDecimalDigits; i++) {
      value.setInteger(i, 0);
    }
    org.apache.drill.common.util.DecimalUtility.getSparseFromBigDecimal(average, value.buffer, value.start, value.scale, value.precision, value.nDecimalDigits);
